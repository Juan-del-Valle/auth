...
type RespondAuthenticatedResponse struct {
...
func (h *Handler) respondAuthenticated(w http.ResponseWriter, r *http.Request, userInfo model.UserInfo) {
	...
	refreshToken, err := h.createRefreshToken(userInfo)
	if err != nil {
		logging.Application(r.Header).WithError(err).Error()
		h.respondError(w, r)
		return
	...
	w.Header().Set("Content-Type", contentTypeJSON)
	w.WriteHeader(200)
	_ = json.NewEncoder(w).Encode(RespondAuthenticatedResponse{
		Token:        token,
...
func (h *Handler) createRefreshToken(userInfo model.UserInfo) (string, error) {
